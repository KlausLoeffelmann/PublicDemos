...
/// <summary>
///  Renders a concave mechanical keytop — the classic table-calculator/cash-register key with a
///  raised rim and a shallow bowl — for arbitrary colors, DPI values and interaction states.
/// </summary>
/// <remarks>
///  <para>
///   The renderer is completely stateless with respect to any control: every input arrives via
///   <see cref="MaterialKeyButtonRenderContext"/>. It can therefore render previews, designer
///   adornments or key visuals inside other controls without a <see cref="MaterialKeyButton"/>
///   instance.
///  </para>
///  <para>
///   Visual layers, back to front: ambient drop shadow, key body (rim surface), concave bowl
///   (path gradient plus inner top-shadow and bottom-light overlays), bowl lip stroke, border,
///   default/focus cues, and finally the caption with a raised or engraved relief.
///  </para>
///  <para>
///   In high-contrast accessibility modes the renderer falls back to a flat, high-contrast style
///   without any material emulation.
///  </para>
/// </remarks>
public sealed class MaterialKeyButtonRenderer
{
    /// <summary>
    ///  Gets a shared default renderer instance. The renderer is stateless and thread-safe.
    /// </summary>
    public static MaterialKeyButtonRenderer Default { get; } = new();
    ...
    public void Render(Graphics graphics, MaterialKeyButtonRenderContext context)
    {
        ...
        {
            RenderHighContrast(graphics, context);
    ...
    /// <summary>
    ///  Calculates the natural size of a key for the given context, including enough room for
    ///  the bowl, rim, border, relief offsets and press-animation motion.
    /// </summary>
    /// <param name="context">The render context (its <c>Bounds</c> are ignored).</param>
    /// <param name="proposedSize">The size proposed by the layout engine.</param>
    public Size GetPreferredSize(MaterialKeyButtonRenderContext context, Size proposedSize)
    {
        ArgumentNullException.ThrowIfNull(context);
        ...
        Size measureProposal = new(int.MaxValue / 2, int.MaxValue / 2);
        Size textSize = string.IsNullOrEmpty(context.Text)
            ? new Size(0, context.Font.Height)
            : TextRenderer.MeasureText(context.Text, context.Font, measureProposal, flags);

        int ambient = Math.Max(1, (int)MathF.Round(2.5f * scale));
        int rim = Math.Max(2, (int)MathF.Round(3f * scale));
        int relief = Math.Max(1, (int)MathF.Round(0.8f * scale));
        int pressMotion = (int)MathF.Ceiling(1.5f * scale);
        int innerMargin = Math.Max(1, (int)MathF.Round(2f * scale));
        int border = Math.Max(0, context.BorderWidth);

        int chrome = 2 * (ambient + border + rim + innerMargin);
        int sideMargin = (int)MathF.Round(7f * scale);
        int capMargin = (int)MathF.Round(3f * scale);

        int width = textSize.Width + context.Padding.Horizontal + chrome + (2 * sideMargin) + relief;
        int height = textSize.Height + context.Padding.Vertical + chrome + (2 * capMargin) + relief + pressMotion;

        // A physical key never degenerates into a flat sliver.
        int minWidth = (int)MathF.Round(44f * scale);
        int minHeight = (int)MathF.Round(32f * scale);
    ...
    private static void DrawBowl(Graphics graphics, Metrics metrics, Palette palette)
    {
        ...
        // 1. Radial shading: edges catch light, the center sits lower and darker — the
        //    signature concave read. The dark center is biased towards the upper left,
        //    where a top-left light source cannot reach into a bowl.
        ...
        // 3. The lip where rim and bowl meet: light on the upper left, shadow lower right —
        //    the raised edge of the surrounding rim.
    ...
    private static void DrawStateCues(
        Graphics graphics,
        MaterialKeyButtonRenderContext context,
        Metrics metrics,
        Palette palette)
    {
        ...
        Palette palette)
    {
        ...
        // The keytop already moves with the press; the caption sinks a touch further,
        // which sells the "finger pushes the key down" moment.
        ...
        MaterialKeyButtonTextEffect effect = context.TextEffect;

        if (effect is not MaterialKeyButtonTextEffect.Flat)
        {
            // GDI text ignores alpha, so the relief colors are opaque blends against the
            // bowl center — see ColorMath.GetTextHighlight/GetTextShadow.
            (Color reliefColor, Point reliefShift) = effect switch
            {
                // Raised: light from the upper left casts the glyph's shadow downwards.
                MaterialKeyButtonTextEffect.Raised => (palette.TextShadow, new Point(0, reliefOffset)),
            ...
            if (effect is MaterialKeyButtonTextEffect.Engraved)
            {
                // A faint dark edge above completes the engraving.
                Rectangle upperRect = textRect;
                upperRect.Offset(0, -reliefOffset);
                Color upperShadow = ColorMath.Blend(palette.TextShadow, palette.BowlCenter, 0.55f);
    ...
    private static void RenderHighContrast(Graphics graphics, MaterialKeyButtonRenderContext context)
    {
        Rectangle bounds = context.Bounds;
        bool pressed = context.RenderState.HasFlag(MaterialKeyButtonRenderState.Pressed);
        float scale = context.DeviceDpi / 96f;
        ...
        Rectangle textRect = Rectangle.Inflate(bounds, -(int)(4 * scale), -(int)(4 * scale));
        textRect = ApplyPadding(textRect, context.Padding);

        if (pressed)
        {
            textRect.Offset((int)scale, (int)scale);
        }
    ...
    /// <summary>
    ///  Creates a rounded-rectangle path; degrades to a plain rectangle for tiny radii and
    ///  clamps the radius so it never exceeds half of the smaller side.
    /// </summary>
    ...
    /// <summary>
    ///  Grows a gradient rectangle by one pixel to avoid GDI+ edge-seam artifacts and to
    ...
    private static TextFormatFlags GetTextFormatFlags(MaterialKeyButtonRenderContext context)
    {
    ...
    private readonly struct Metrics
    {
        ...
        public static Metrics Create(MaterialKeyButtonRenderContext context)
        {
            MaterialKeyButtonRendererOptions options = context.Options;
            Rectangle bounds = context.Bounds;
            ...
            // Pressing sinks the keytop; the bottom edge stays put, so the cap compresses.
            ...
            // Pressing deepens the bowl; hovering flattens it a touch, as if the key rises
            // to meet the finger. Highlights brighten on hover, shadows deepen on press.
    ...
    /// <summary>
    ///  All colors of one render pass, derived from the effective context colors so the
    ///  material effect adapts to any <c>BackColor</c>/<c>ForeColor</c> combination.
    /// </summary>
    private readonly struct Palette
    {
        ...
        public static Palette Create(MaterialKeyButtonRenderContext context, Metrics metrics)
        {
            bool enabled = context.Enabled;

            // Disabled keys mute the material but keep the concave form readable —
            // reduced contrast rather than flat gray.
            Color back = enabled ? context.BackColor : ColorMath.Mute(context.BackColor, 0.55f);
            float contrast = enabled ? 1f : 0.35f;
            float luminance = ColorMath.GetLuminance(back);

            Color border = enabled
                ? context.BorderColor
                : ColorMath.Blend(context.BorderColor, back, 0.45f);

            // Keep the border visible even if the user picked one too close to the face color.
            border = ColorMath.EnsureContrast(border, back, 0.08f);

            Color text = enabled
                ? context.ForeColor
                : ColorMath.EnsureContrast(ColorMath.Blend(context.ForeColor, back, 0.45f), back, 0.18f);

            float reliefStrength = (enabled ? 1f : 0.4f) + (metrics.Press * 0.25f);
            Color bowlCenter = ColorMath.Darken(back, metrics.ShadowAmount * 0.75f * contrast + (enabled ? 0f : 0.02f));

            return new Palette
            {
                BodyLight = ColorMath.Lighten(back, metrics.HighlightAmount * 0.9f * contrast),
                BodyDark = ColorMath.Darken(back, metrics.ShadowAmount * 0.9f * contrast),
                BowlEdge = ColorMath.Lighten(back, metrics.HighlightAmount * 0.55f * contrast),
                BowlCenter = bowlCenter,
                LipLight = ColorMath.Lighten(back, metrics.HighlightAmount * 1.3f * contrast),
                LipDark = ColorMath.Darken(back, metrics.ShadowAmount * 1.2f * contrast),
                Border = border,
                Text = text,
                TextHighlight = ColorMath.GetTextHighlight(bowlCenter, reliefStrength),
                TextShadow = ColorMath.GetTextShadow(bowlCenter, reliefStrength),
                Focus = ColorMath.TowardsContrast(back, 0.55f),
                DefaultCue = ColorMath.TowardsContrast(border, 0.3f),
