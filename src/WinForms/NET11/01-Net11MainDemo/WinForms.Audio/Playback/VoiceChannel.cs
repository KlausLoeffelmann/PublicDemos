...
/// <remarks>
///  <para>
///   All Play* tasks complete when the voice has rendered its last sample (release included).
///   Queued device audio and the shared reverb tail can still be sounding. Cancelling the
///   token doesn't throw; it releases the voice early and the task completes normally once rendered.
...
public sealed class VoiceChannel
{
    ...
    /// </summary>
    /// <remarks>
    ///  This path does not allocate a completion task per voice. Observe
    ///  <see cref="AudioEngine.Completion"/> for engine failures, including failures after a
    ///  trigger was accepted. The task belongs to the engine, not to an individual voice.
    /// </remarks>
    public void Trigger(IVoice voice)
        => _engine.Trigger(voice, ReverbSend);
