...
/// </summary>
public static partial class HighPrecisionTimer
{
    ...
    /// </summary>
    public static bool IsHighResolutionAvailable => s_highResolutionAvailable;
    ...
    /// </exception>
    public static TimerRegistration Register(Func<HighPrecisionTimerTick, CancellationToken, ValueTask> callback)
    {
        ...
        var registration = new Registration(id, callback, syncContext);
    ...
    /// </summary>
    public static void Unregister(long registrationId)
    {
    ...
    {
        using var periodicTimer = new PeriodicTimer(TimeSpan.FromMilliseconds(s_timerTickMs));
        ...
        try
        {
            while (await periodicTimer.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                elapsed = currentTimestamp - lastTickTimestamp;
        ...
        CancellationToken cancellationToken)
    {
        foreach (var kvp in s_registrations)
        {
            ...
            if (Interlocked.CompareExchange(ref reg.InFlight, 1, 0) != 0)
            {
                Interlocked.Increment(ref reg.DroppedFrames);
                continue;
            }

            long frameIndex = Interlocked.Increment(ref reg.FrameIndex) - 1;
            int dropped = Interlocked.Exchange(ref reg.DroppedFrames, 0);
            ...
            reg.SyncContext.Post(_ =>
            {
                _ = InvokeCallbackAsync(reg, tick, cancellationToken);
            }, null);
        }
    }
    ...
    #region Timer Resolution P/Invoke
    ...
    private static bool TrySetHighResolutionTimerMode()
    {
        ...
        try
        {
            ...
            int result = NativeMethods.TimeBeginPeriod(1);
            return result == 0; // TIMERR_NOERROR
        }
        catch
        {
            return false;
    ...
    /// </summary>
    public readonly struct TimerRegistration : IDisposable
    {
